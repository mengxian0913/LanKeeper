...
const Quize = () => {
  ...
  const NextQuestion = (title:string,ans:string,key:number,anskey:number) => {
    console.log();
    if(title==ans) {
    HandlerememberValue(key,true)
    Setcorrect(correct+1)
    } 
    else{HandlewrongAns(key,anskey)}
  ...
  const HandlewrongAns = (key:number,anskey:number) => {
    ...
    const newData = {...wrongVoc,choosen:jsondata[key].word,correct:jsondata[anskey].word,description:jsondata[anskey].description}
  ...
  const HandleWeights = (NumArray) => {
    var totalweights=0;
    var reNum:number;
    jsondata.map((item)=>{totalweights+=item.rememberValue});
    ...
    for(var i:number=0;i<amountOfVoc;i++)
    {
      ...
      if(randonNum < countWeights&& i!=NumArray[0].num && i!=NumArray[1].num && i!=NumArray[2].num && i!=NumArray[3].num){
        reNum = i;
        return reNum;
      }
    }
  ...
  const TestResult = () => {
    ...
    <View style={{ width: screenWidth * 0.95, alignItems: "center",marginHorizontal:-9}}>
      ...
      <View style={styles.questionNumSelector}>
        ...
        <Text style={{ fontSize: 24,fontWeight: "500" }}>correct: {item.correct}</Text>
        <Text style={{ fontSize: 24,fontWeight: "500" }}>description: {item.description}</Text>
      </View>
      ))}
    </View>
    </ScrollView>
  ...
  const NotInTest = () => {
    ...
    <View style={{marginVertical:15,width: screenWidth, paddingHorizontal: 18, paddingVertical: 20 }}>
      <View style={styles.questionNumSelector}>
        <TouchableOpacity>
          <Text style={{ fontSize: 24, fontWeight: "500" }} onPress={FiveQuestion}>Five Questions</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.questionNumSelector}>
        <TouchableOpacity>
          <Text style={{ fontSize: 24, fontWeight: "500" }} onPress={TenQuestion}>Ten Questions</Text>
        </TouchableOpacity>
      </View>
        <View style={styles.questionNumSelector}>
          <TouchableOpacity>
            <Text style={{ fontSize: 24, fontWeight: "500" }} onPress={TwentyQuestion}>Twenty Questions</Text>
        </TouchableOpacity>
      </View>
      </View>
      )
  ...
  const RenderItem = ({AnswerNum,item}) => {
    return(
      <Text style={{ fontSize: 24, fontWeight: "500" }}  onPress={() => NextQuestion(jsondata[AnswerNum].word,jsondata[item.num].word,item.num,AnswerNum)}>{jsondata[item.num].word}</Text>
    )
  }
  const InTest = () => {
